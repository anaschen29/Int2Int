...
class EnvDataset(Dataset):
    def __init__(self, env, task, train, params, path, size=None, type=None):
        ...
        if path is not None:
            ...
            if params.batch_load and self.train:
                ...
                #logger.info(f"{self.data[0]}\n{self.data[1]}")
                #logger.info(f"{self.data[0][0].split()} {self.data[0][1].split()}" )
    ...
    def collate_fn(self, elements):
        ...
        if self.decoder_only:
            ...
            y, y_len = self.batch_sequences(y, self.env.pad_index, self.env.eos_index, self.env.eos_index, True if self.encoder_only else False)
    ...
    def read_sample(self, index):
        ...
        if self.train:
            if self.batch_load:
                ...
                if self.two_classes:
                    if self.env.rng.rand()< self.first_class_prob:
                        idx = (self.env.rng.randint(self.first_class_size)) % len(self.data)
                    else:
                        idx = (self.first_class_size + self.env.rng.randint(len(self.data) - self.first_class_size)) % len(self.data)
                else:
