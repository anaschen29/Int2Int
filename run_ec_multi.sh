...
NGPU=8
ROOT="../Math4AI"
DATA="${ROOT}/ec_data/"             # .folder containing  the data
RUNS="${ROOT}/ec_experiment_runs"

# Environment / data flags
ENV_FLAGS="--env_name arithmetic --operation elliptic --base 10"
EVAL_DATA="--eval_data ${DATA}/valid.txt,${DATA}/test.txt,${DATA}/robust.txt"
...
EVAL_SIZE=-1                        # during training; do -1 later in eval-only
...
SEEDS=(1982 2010 2014)                       # 2 seeds keep variance visible without doubling wall time too much
MIXES=(orig)                # train_data variants
DEPTHS=("4x4" "6x6")                    # encoder/decoder depth pairs
LRS=(0.0001 0.0005 0.001)                     # two learning rates
DROPS=(0.0)                         # model dropout
